...
{counter name="tabindex" assign=tabindex print=false}
{if $def.type != "hidden" AND !$def.hidden}
  <tr>
    <td class="property">{if $def.title}{$def.title}{else}{$header|replace:"_":" "|capitalize}{/if}{if $def.guidance}<br /><small>{$def.guidance}</small>{/if}</td>
